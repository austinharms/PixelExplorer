...
void BaseBlock::LoadBlockManifest() {
	...
	if (!manifest || manifest.peek() == std::ifstream::traits_type::eof()) {
		std::cout << "No Block Manifest Found, Creating Empty Manifest" << std::endl;
		BaseBlock::s_blockCount = 0;
	...
	assert(LoadPackage("default", &loadedPackages));
	for (const std::string& package : World::GetPackages())
...
bool BaseBlock::LoadPackage(std::string name, std::set<std::string>* loadedPackages) {
	// Dont load package if its already loaded
	if (loadedPackages != nullptr && !loadedPackages->empty() &&
		loadedPackages->find(name) != loadedPackages->end())
		return true;
