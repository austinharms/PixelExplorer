	...
	for (uint32_t i = 0; i < Chunk::BLOCK_COUNT; ++i) {
		...
		if (!curBlock.isEmpty()) {
			...
			if (z != Chunk::CHUNK_SIZE - 1) {
				...
				otherBlock.setChunkBlock(&_adjacentChunks[(uint8_t)FaceDirection::BACK]->_blocks[i - Chunk::LAYER_SIZE], i - Chunk::LAYER_SIZE, _adjacentChunks[(uint8_t)FaceDirection::BACK]);
			}
