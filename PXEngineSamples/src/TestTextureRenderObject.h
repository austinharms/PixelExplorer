...
{
public:
	TestTexturedRenderObject(pxengine::PxeRenderMaterial& material) : pxengine::PxeRenderObject(material) {
		...
		pxengine::PxeBuffer* indexBuffer = new(std::nothrow) pxengine::PxeBuffer(sizeof(indices));
		memcpy(indexBuffer->getBuffer(), indices, indexBuffer->getSize());
		pxengine::PxeBuffer* vertexBuffer = new(std::nothrow) pxengine::PxeBuffer(sizeof(vertices));
	...
	virtual ~TestTexturedRenderObject() {
		_indexBuffer->drop();
		_vertexArray->drop();
		_vertexBuffer->drop();
	}
