...
class PhysicsRenderable : public pxengine::PxePhysicsRenderObject
{
public:
	static PhysicsRenderable* createPhysicsRenderable(const glm::vec3& pos, bool dynamic, pxengine::PxeRenderMaterial& material, pxengine::PxeIndexBuffer* indexBuffer = nullptr, pxengine::PxeVertexArray* vertextArray = nullptr, physx::PxShape* shape = nullptr, const glm::vec3& velocity = glm::vec3(0)) {
		...
		PhysicsRenderable* renderable = new(std::nothrow) PhysicsRenderable(*actor, material, indexBuffer, vertextArray, shape);
		renderable->positionMatrix = glm::translate(renderable->positionMatrix, pos);
		return renderable;
	}

	virtual ~PhysicsRenderable() {
		_indexBuffer->drop();
		_vertexArray->drop();
		_pxShape->release();
	}
...
private:
	PhysicsRenderable(physx::PxRigidActor& actor, pxengine::PxeRenderMaterial& material, pxengine::PxeIndexBuffer* indexBuffer, pxengine::PxeVertexArray* vertextArray, physx::PxShape* shape) : pxengine::PxePhysicsRenderObject(material, actor) {
		uint16_t indices[36] = {
			0, 2, 1, 0, 3, 2,  // Front
			4, 5, 6, 4, 6, 7,  // Back
			0, 4, 7, 0, 7, 3,  // Left
		...
		if (shape) {
			...
			//_pxShape = physics->createShape(PxTriangleMeshGeometry(mesh), *material);
			//mesh->release();
			//material->release();
