...
namespace pecore::graphics::open_gl {
	struct Window : public PE_Window {
		...
		static Window* CreateWindow(const char* title, int width, int height, Uint32 flags) PE_NOEXCEPT {
			...
			new(window) Window();
			PE_EventLoop.PushBlockingTWork<CreateWindowHandle>(window, title, width, height, flags);
		...
		void DestroyWindow() PE_NOEXCEPT {
#if PE_GL_CONTEXT_PER_WINDOW
			worker.PushBlockingTWork<DestroyContext>(this);
#endif
			PE_EventLoop.PushBlockingTWork<DestroyWindowHandle>(this);
			this->~Window();
			PE_free(this);
		}
	...
	protected:
...
#else
			if (SDL_GL_MakeCurrent(window_handle, cur_ctx) != 0) {
				PE_LogError(PE_LOG_CATEGORY_RENDER, PE_TEXT("Failed to create window, context incompatable with window, error: %s"), SDL_GetError());
				SDL_DestroyWindow(window_handle);
				window_handle = nullptr;
			}
#endif
		...
		void DestroyContext() {
			int rtn = SDL_GL_DeleteContext(context.opengl_context);
			if (rtn != 0) {
