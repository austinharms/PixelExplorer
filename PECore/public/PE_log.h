...
namespace pe {
	...
	// Called when an assert fails or a critical log occurs, it will immediately exit the application
	PE_EXTERN_C PE_NORETURN PE_API void PE_CALL AbortApplication();

	// TODO Replace this with custom log implementation
...
#define PE_LogCritical(category, fmt, ...) do { SDL_LogCritical(category, fmt, __VA_ARGS__); pe::AbortApplication(); } while(false)
