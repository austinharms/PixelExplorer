...
namespace pxengine {
	namespace nonpublic {
		...
		void NpWindow::initializeAsset(PxeGLAsset& asset)
		{
			bool boundContext = !_acquiredContext;
			if (boundContext) acquireGlContext();
			_engineBase->initializeGlAsset(asset);
			if (boundContext) releaseGlContext();
		}
